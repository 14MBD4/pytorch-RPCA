...
class RPCA:
    def __init__(self, matrix, mu=None, lmd=None):
        ...
        self.__lmd = lmd if lmd else (
                torch.tensor(1).cuda() / torch.sqrt(torch.max(torch.tensor(self.__M.shape).cuda())))
        self.__delta = torch.tensor(1e-7).cuda()
    ...
    def singular_value_threshold(self, matrix):
        ...
        s = torch.diag(self.shrinkage(sigma))  # Thresholding of singular values and construction of diagonal matrices
        return torch.matmul(torch.matmul(u, s), v)

    def shrinkage(self, matrix):
        return torch.sign(matrix) * torch.max((torch.abs(matrix) - (self.__lmd * self.__mu)),
                                              torch.zeros(matrix.shape).cuda())
