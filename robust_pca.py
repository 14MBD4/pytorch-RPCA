...
class RPCA:
    def __init__(self, matrix, mu=None, lmd=None):
        ...
        self.__mu = (
            ...
            if mu
                ...
                torch.prod(torch.tensor(self.__M.shape).cuda())
        ...
        self.__lmd = (
            ...
            if lmd
                ...
                torch.tensor(1).cuda()
                / torch.sqrt(torch.max(torch.tensor(self.__M.shape).cuda()))
            )
        )
    ...
    def alternating_directions(self, max_iter_time=500):
        L2norm_value = torch.tensor(
            1
        ).cuda()  # The initialized 1 is large enough for 'lowerlimitvalue' to enter the judgment
